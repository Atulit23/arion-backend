...
import { JwtAuthGuard } from 'src/auth/jwt/jwt-auth.guard';
  ...
  export class DocumentsController {
    ...
    @UseGuards(JwtAuthGuard)
    async getDocumentsByUserId(@Query('userId') userId: string, @Res() res: Response) {
        return await this.documentsService.getDocumentsByUserId(userId, res);
    }

    @Get('getDocumentsForProgress')
    @UseGuards(JwtAuthGuard)
    async getDocumentsForProgress(@Query('userId') userId: string, @Query('documentId') documentId: string, @Res() res: Response) {
        return await this.documentsService.getDocumentsForProgress(userId, documentId, res);
    }
  }
